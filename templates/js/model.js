...
function toggleTreeEntry(listItem) {
	...
	if (isTreeFiltered()) {
		...
		if (children.is(":visible")) {
			children.hide('fast');
			// Toggle arrow icon
			$(listItem).find('> span > i').addClass('glyphicon-chevron-right').removeClass('glyphicon-chevron-down');
		} else {
			children.show('fast');
			// Toggle arrow icon
			$(listItem).find('> span > i').addClass('glyphicon-chevron-down').removeClass('glyphicon-chevron-right');
		}
	}
}
